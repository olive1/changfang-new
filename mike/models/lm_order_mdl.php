...
/** 用户会员模型   */
class Lm_order_mdl extends CI_Model{
    
    const   LM_ORDER =   'LM_ORDER';   
    ...
    function add($data){
        ...
        $query      =   $this->db->insert(self::LM_ORDER,    $data);
    ...
    function del($ary){
        ...
        $this->db->delete(self::LM_ORDER, $ary);
        return ($this->db->affected_rows() == 1)    ?   TRUE    :   FALSE;
    }
    ...
    function update($data, $ary){
        ...
        $this->db->where($ary);
        $this->db->update(self::LM_ORDER, $data);
    ...
    function select($ary){
        ...
        $this->db->where($ary);
        $query  =   $this->db->get(self::LM_ORDER);   
