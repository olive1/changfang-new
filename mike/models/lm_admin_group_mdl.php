...
/** 用户会员模型   */
class Lm_admin_group_mdl extends CI_Model{
    
    const   LM_ADMIN_GROUP =   'lm_admin_group';   
    ...
    function add($data){
        ...
        $query      =   $this->db->insert(self::LM_ADMIN_GROUP,    $data);
    ...
    function del($ary){
        ...
        $this->db->delete(self::LM_ADMIN_GROUP, $ary);
    ...
    function update($data, $ary){
        ...
        $this->db->where($ary);
        $this->db->update(self::LM_ADMIN_GROUP, $data);
    ...
    function select($ary){
        ...
        $this->db->where($ary);
        $query  =   $this->db->get(self::LM_ADMIN_GROUP);   
