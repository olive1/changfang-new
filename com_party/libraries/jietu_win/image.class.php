...
class Zubrag_image{
    ...
    function SaveImage($im, $filename) {
        ...
        if (($this->image_type == 1) && !function_exists('imagegif')) $this->image_type = 3;// endif
    ...
    function ImageCreateFromType($type, $filename) {
        ...
        function GenerateThumbFile($from_name, $to_name) {
            ...
            if ($this->cut_x > 0) $orig_x = min($this->cut_x, $orig_x);//end 
            if ($this->cut_y > 0) $orig_y = min($this->cut_y, $orig_y);//end 

            // should we override thumb image type?
            $this->image_type = ($this->image_type != - 1 ? $this->image_type : $orig_img_type);

            // check for allowed image types
            if ($orig_img_type < 1 or $orig_img_type > 3) die("Image type not supported");//end 
