...
/** 总分类模型   */
class Lm_menu_mdl extends CI_Model{
    
    const   LM_MENU =   'lm_menu';
    public  $lists      =   '';//存储
    ...
    function add($data){
        ...
        $this->db->insert(self::LM_MENU,    $data);
    ...
    function del($ary){
        ...
        $this->db->delete(self::LM_MENU, $ary);
    ...
    function update($data, $ary){
        ...
        $this->db->where($ary);
        $this->db->update(self::LM_MENU, $data);
    ...
    function select($ary){
        ...
        $this->db->where($ary);
        $query  =   $this->db->get(self::LM_MENU);   
