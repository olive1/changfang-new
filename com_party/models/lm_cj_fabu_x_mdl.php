...
/** 用户会员模型   */
class Lm_cj_fabu_x_mdl extends CI_Model{
    
    const   LM_CJ_FABU_X =   'lm_cj_fabu_x';   
    ...
    function add($data){
        ...
        $query      =   $this->db->insert(self::LM_CJ_FABU_X,    $data);
    ...
    function del($ary){
        ...
        $this->db->delete(self::LM_CJ_FABU_X, $ary);
    ...
    function update($data, $ary){
        ...
        $this->db->where($ary);
        $this->db->update(self::LM_CJ_FABU_X, $data);
    ...
    function select($ary){
        ...
        $this->db->where($ary);
        $query  =   $this->db->get(self::LM_CJ_FABU_X);   
    ...
    function count_all_results($ary){	
        if($ary)$this->db->where($ary);
		$this->db->from(self::LM_CJ_FABU_X);
