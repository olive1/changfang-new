...
class LM_Upload {
	...
	/**
	*它后面跟着了6个参数,好像一下子看晕了.其中
	
	第一个参数 是来自view层中input的表单的name值,
	第二个参数 用$_file(userfile)即取得上传文件的一些信息.
	第三个参数 是个bool值就是问你是否是多文件上传,它那里默认是false,即默认不是多文件上传.
	第四个参数 大家应该一看就知道了$upload_path就表示上传的文件的路径,路径是相对index.php而言的这里要注意,
		       并且路径中的文件夹必须是真实的文件,所以你得在那个路径下建立相应的文件夹.
	第五个参数 则是缩微图的路径,
	第六个参数 即最后一个参数即是问你时候需要生成缩微图.它默认也是不生成缩微图的.  
	*/
	
	
	function do_upload_ex($field = 'userfile',$uploaddata,$Multi_file=false,$upload_path,$upload_path_thumb="/uploads/thumb/",$pro_thumb=false)
	{
		$CI =& get_instance();
		$imgname = array();
		...
		$config['allowed_types']   = LUMN_ALLOWED_TYPES;
		$config['max_size']        = LUMN_MAX_SIZE;
		$config['max_width']       = LUMN_MAX_WIDTH;
		$config['max_heigth']      = LUMN_MAX_HEIGHT;
		...
		foreach($_FILES as $key => $value){
			if( ! empty($key['name'])){
				if(!$this->do_upload($key)){ 		
					...
					echo "<script language='JavaScript'>alert('你没有选择文件，或者文件大小超过".LUMN_MAX_SIZE."KB');history.back();</script>";
                    
                    //$this->session->set_flashdata('success', '你没有选择文件！');
					//go_back();
                    exit;
				}else{
					...
					if($pro_thumb){				//缩略图
                        ...
                        foreach($imgname as $key => $value)//遍历生成缩略图。
                        {
                            $CI->load->library('image_lib');
    						$config['image_library'] = 'gd2';
    						$config['source_image'] = '.'.$upload_path.$imgname[$key];
    						$config['new_image'] = '.'.$upload_path_thumb.$imgname[$key];
    						$config['thumb_marker'] = '';
    						$config['create_thumb'] = TRUE;
    						$config['maintain_ratio'] = TRUE;
    						$config['width'] = LUMN_WIDTH;
                            $config['height'] = LUMN_HEIGHT;
